...
namespace OZONEOnigiri.Modules
{
	public class Information : BaseCommandModule
	{
		...
		public async Task Leaderboard(CommandContext ctx)
		{
			...
			foreach (var user in Onigiri._db.Users.OrderByDescending(u => u.Level).ThenBy(u => u.Experience).Take(10))
			{
				string medalIcon = "";
		...
		{
			if (Onigiri._db.Users.Any(u => u.Id == ctx.User.Id))
			{
				DiscordEmbedBuilder embedBuilder = new DiscordEmbedBuilder();
				...
				OnigiriUser user = Onigiri._db.Users.Single(u => u.Id == ctx.User.Id);
